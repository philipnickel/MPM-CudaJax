from types import SimpleNamespace

import jax.numpy as jnp
import jax
from omegaconf import OmegaConf
from mpm_jax.types import MPMState, MPMParams
from mpm_jax.p2g.backends import JaxBackend
from mpm_jax.solver import MPMSolver, RuntimeConfig
from mpm_jax.constitutive import stvk_elasticity_jacobi


def _runtime_params(n_particles, num_grids, dt=3e-4):
    return MPMParams(
        OmegaConf.create(
            {
                "n_particles": n_particles,
                "num_grids": num_grids,
                "dt": dt,
                "gravity": [0.0, 0.0, -9.8],
                "rho": 1000.0,
                "clip_bound": 0.5,
                "damping": 1.0,
                "size": [1.0, 1.0, 1.0],
            }
        )
    )


def _solver(n_particles, num_grids, steps_per_frame, dt=3e-4):
    sim = OmegaConf.create(
        {
            "n_particles": n_particles,
            "num_grids": num_grids,
            "dt": dt,
            "gravity": [0.0, 0.0, -9.8],
            "rho": 1000.0,
            "clip_bound": 0.5,
            "damping": 1.0,
            "size": [1.0, 1.0, 1.0],
            "initial_velocity": [0.0, 0.0, -0.5],
            "center": [0.5, 0.5, 0.5],
            "steps_per_frame": steps_per_frame,
        }
    )
    material = SimpleNamespace(elasticity=stvk_elasticity_jacobi(E=2e6, nu=0.4))
    return MPMSolver(RuntimeConfig(material=material, sim=sim, backend=JaxBackend()))


def test_elastic_simulation_10_frames():
    N = 100
    num_grids = 15
    x0 = jnp.ones((N, 3)) * 0.5
    params = _runtime_params(n_particles=N, num_grids=num_grids, dt=3e-4)
    state = MPMState(
        x=x0,
        v=jnp.broadcast_to(jnp.array([0.0, 0.0, -0.5]), (N, 3)).copy(),
        C=jnp.zeros((N, 3, 3)),
        F=jnp.tile(jnp.eye(3), (N, 1, 1)),
    )
    solver = _solver(N, num_grids, steps_per_frame=5, dt=params.dt)
    solver.state = state
    for _ in range(10):
        state = solver._frame(solver.state)
        solver.state = state
    jax.block_until_ready(state.x)
    assert jnp.mean(state.x[:, 2]) < 0.5
    assert jnp.all(jnp.isfinite(state.x))


def test_outer_frame_jit_runs_multiple_frames():
    N = 200
    num_grids = 16
    steps_per_frame = 4
    x0 = jnp.ones((N, 3), dtype=jnp.float32) * 0.5

    params = _runtime_params(n_particles=N, num_grids=num_grids, dt=3e-4)

    state = MPMState(
        x=x0,
        v=jnp.broadcast_to(jnp.array([0.0, 0.0, -0.5]), (N, 3)).copy(),
        C=jnp.zeros((N, 3, 3)),
        F=jnp.tile(jnp.eye(3), (N, 1, 1)),
    )
    solver = _solver(N, num_grids, steps_per_frame=steps_per_frame, dt=params.dt)
    solver.state = state
    state = solver._frame(solver.state)
    solver.state = state
    state = solver._frame(solver.state)
    solver.state = state
    jax.block_until_ready(state.x)

    assert state.x.shape == (N, 3)
    assert state.v.shape == (N, 3)
    assert state.F.shape == (N, 3, 3)
    assert jnp.all(jnp.isfinite(state.x))
