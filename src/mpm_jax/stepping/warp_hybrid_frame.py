...
def _jax_callable_graph_mode(name):
    modes = {
    ...
    try:
        return modes[str(name).lower()]
    except KeyError as exc:
        raise ValueError(
            f"Unknown warp_v3_supercell_tile graph_mode={name!r}; "
            f"expected one of {sorted(modes)}."
        ) from exc


def _make_jax_p2g_supercell_tile(graph_mode):
    return jax_callable(
