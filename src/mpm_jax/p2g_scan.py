...
def _single_particle_one_stencil(
    x_p, v_p, C_p, stress_p, offset_int, dt, vol, p_mass, dx, inv_dx, num_grids
):
    ...
    mv_s = -dt * vol * (stress_p @ dweight) + p_mass * weight * (v_p + C_p @ dpos)
    m_s = weight * p_mass
...
def _p2g_scan(x, v, C, stress, dt, vol, p_mass, dx, inv_dx, num_grids):
    ...
    (grid_m, grid_mv), _ = jax.lax.scan(scan_body, (grid_m0, grid_mv0), OFFSET_27)
    # TODO: instead of lax scanning here can we express it as a array operation?
