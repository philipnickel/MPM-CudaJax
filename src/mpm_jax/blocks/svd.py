...
def _svd_single(F, num_sweeps=4):
    """SVD of a SINGLE 3x3 matrix (apply to batches with ``jax.vmap``).

    S (symmetric) and V are carried as 3x3 nested lists of scalar leaf arrays
    rather than (3, 3) tensors, so the Givens rotations are plain elementwise
    assignments with no ``.at[].set()``. Under ``vmap`` each leaf becomes a
    (...,) array and XLA fuses the whole decomposition into one register-resident
    kernel -- instead of the ~60 ``dynamic-update-slice`` launches a tensor-valued
    formulation emits (each round-tripping S, V through HBM). 4 sweeps (= 12
    rotations) is enough for f32 on physically reasonable deformation gradients
    (McAdams 2011). Returns (U, sigma, Vh = V^T), matching ``jnp.linalg.svd``.
    """
    one, zero = jnp.ones((), F.dtype), jnp.zeros((), F.dtype)
    FtF = F.T @ F
    ...
    sigma = jnp.sqrt(jnp.maximum(jnp.stack([S[0][0], S[1][1], S[2][2]]), 0.0))
    Vmat = jnp.stack([jnp.stack(row) for row in V])
    inv_sigma = jnp.where(sigma > 1e-10, 1.0 / sigma, 0.0)
    U = (F @ Vmat) * inv_sigma  # scale columns of F V by 1/sigma
    return U, sigma, Vmat.T


def jacobi_svd_3x3(F, num_sweeps=4):
    """Batched SVD of (..., 3, 3): ``jax.vmap`` of the single-matrix Jacobi.

    Flattens arbitrary leading batch dims, vmaps ``_svd_single`` over them, and
    unflattens. Returns (U, sigma, Vh = V^T), matching
    ``jnp.linalg.svd(F, full_matrices=False)``.
    """
    batch = F.shape[:-2]
    U, sigma, Vh = jax.vmap(lambda M: _svd_single(M, num_sweeps))(
        F.reshape(-1, 3, 3)
    )
    return (
        U.reshape(*batch, 3, 3),
        sigma.reshape(*batch, 3),
        Vh.reshape(*batch, 3, 3),
    )
