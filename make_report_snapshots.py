...
def build_cfg(args):
    ...
    with initialize_config_dir(
        version_base=None, config_dir=os.path.join(REPO, "conf")
    ):
        cfg = compose(
            config_name="config",
            overrides=[
...
def run_arc(cfg, args, target_frames=None):
    ...
    print(
        f"Building solver: backend={solver.backend.name} n_particles={n:,} "
        f"num_grids={cfg.sim.num_grids} dx={1.0 / float(cfg.sim.num_grids):.5f} dt={dt:g} "
        f"steps/frame={spf} frames={cfg.sim.num_frames}"
    )
    print(
        f"  block center={ctr} size={sz} -> lateral margin to edge ~{margin:.3f} "
    ...
    def metrics(state):
        z = state.x[:, 2]
        return (float(z.min()), float(z.mean()), float(jnp.abs(state.v).max()))
...
def select_frames(metrics_arr, times, cfg, args):
    ...
    if args.select:
        ...
        return [
            (min(max(i, 0), nframes), ROLES[k] if k < len(ROLES) else f"frame-{i}")
    ...
    win = (
        int(round(args.impact_window / frame_dt)) if frame_dt > 0 else (nframes - onset)
    )
    ...
    if len(seg) and float(seg.max()) > args.v_cap:
        ...
        print(
            f"  [warn] v_max exceeds {args.v_cap} at frame {bad} "
...
def render_snapshot(points, out_path, *, label, t, point_size):
    ...
    floor = pv.Plane(
        center=(0.5, 0.5, 0.02), direction=(0, 0, 1), i_size=1.0, j_size=1.0
    )
...
def _enrich_usd_realism(usd_path, sand_rgb=(0.80, 0.64, 0.42)):
    ...
    the sand + floor and tune the sky dome / warm angled sun."""
...
def _set_rtx_background(usd_path, product_path, sky_rgb=(0.62, 0.70, 0.85)):
    ...
    def author(prim):
        ...
        prim.CreateAttribute(
            "omni:rtx:background:source:color", Sdf.ValueTypeNames.Color3f
        ).Set(Gf.Vec3f(*sky_rgb))
...
def render_selection_ovrtx(clouds, labels, times, args):
    ...
    S.visualize_frames_warp_usd(
        frames, usd_path, color=args.color, radius=args.radius, fps=1
    )
    if args.realism:
    ...
    print(
        f"Creating OVRTX renderer (first run compiles shaders); render {rw}x{rh} -> {w}x{h}..."
    )
    ...
    for n, (label, t) in enumerate(zip(labels, times), 1):
        ...
        print(
            f"  [{n}/{len(clouds)}] {out}  ({os.path.getsize(out) // 1024} KB)  "
...
def render_selection_opengl(clouds, labels, times, args):
    ...
    renderer = wp.render.OpenGLRenderer(
        ...
        far_plane=10.0,
        camera_fov=32.0,
        camera_pos=OPENGL_CAMERA_POS,
        camera_front=OPENGL_CAMERA_FRONT,
...
def main():
    ...
    ap.add_argument(
        ...
        type=int,
        default=0,
        help="override num_grids (0=benchmark default 124)",
    )
    ap.add_argument(
        "--particles",
        type=int,
        default=0,
        help="override n_particles (0=benchmark default 8M)",
    )
    ap.add_argument(
        "--dt", type=float, default=0.0, help="override dt (0=benchmark default 5e-5)"
    )
    ap.add_argument(
        "--size",
        type=float,
    ...
    ap.add_argument(
        "--radius", type=float, default=0.0045, help="ovrtx sphere radius (world units)"
    )
    ...
    ap.add_argument(
        "--no-render", action="store_true", help="run + select only; skip PNGs"
    )
    ap.add_argument(
        "--from-npz",
        type=str,
    ...
    if args.n_snapshots and args.n_snapshots > 0:
        target_frames = sorted(
            {int(round(v)) for v in np.linspace(0, nframes, int(args.n_snapshots))}
        )
    clouds, metrics_arr, times, frame_dt = run_arc(
        cfg, args, target_frames=target_frames
    )
    ...
    with open(csv_path, "w") as fh:
        ...
        for k in range(len(times)):
            fh.write(
                f"{k},{times[k]:.6f},{metrics_arr[k, 0]:.6f},"
    ...
    for i, label in chosen:
        print(
            f"  frame {i:4d}  t={times[i] * 1000:7.1f} ms  "
    ...
    np.savez_compressed(
        npz_path, clouds=sel_clouds, labels=np.array(sel_labels), times=sel_times
    )
