"""Pure JAX P2G baseline using MLS-MPM Q_p formulation (Hu et al. 2018).

Per particle, computes one affine matrix:
    Q_p = dt * vol * 4 * inv_dx^2 * stress + p_mass * C

Per node, one mat-vec:
    mv_i = weight_i * (p_mass * v + Q_p @ dpos_i)

No weight gradient (dweight) needed. Simpler, fewer FLOPs.
"""
import functools
import jax
...
def make_jax_p2g(cfg):
    ...
    def _single_particle_scatter(v_p, Q_p, weight, dpos, p_mass):
        """MLS-MPM P2G: mv_i = weight_i * (p_mass * v + Q @ dpos_i)"""
        mv = weight[:, None] * (p_mass * v_p[None, :] + (Q_p @ dpos.T).T)
        m = weight * p_mass
        return mv, m
    ...
    def _p2g_jitted(state, num_grids, inv_dx, dx, dt, vol, p_mass):
        ...
        stress = elasticity_fn(F)  # Kirchhoff stress tau (N, 3, 3)

        # Q_p = dt * vol * 4 * inv_dx^2 * tau + p_mass * C  (N, 3, 3)
        Q = dt * vol * 4.0 * inv_dx * inv_dx * stress + p_mass * state.C

        weight, dpos, index = compute_weights(state.x, inv_dx, dx, num_grids)
        mv, m = scatter_vmap(state.v, Q, weight, dpos, p_mass)
