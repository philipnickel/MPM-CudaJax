"""Pure JAX P2G using MLS-MPM Q_p formulation (Hu et al. 2018).

Per particle, one affine matrix:
    Q_p = dt * vol * 4*inv_dx^2 * stress + p_mass * C

Per node, one mat-vec:
    mv_i = weight_i * (p_mass * v + Q_p @ dpos_i)

No weight gradient (dweight) needed anywhere.
"""
import functools
import jax
...
def make_jax_p2g(cfg):
    ...
    def _p2g_jitted(state, num_grids, inv_dx, dx, dt, vol, p_mass):
        ...
        # Q_p = dt * vol * 4*inv_dx^2 * tau + p_mass * C
