"""Order-invariant equivalence check: cuda_v4_inline vs cuda_v1_inline.

Same shape as scripts/check_v3_equivalence.py — v4_inline sorts particles by
super-cell, so the output state is in sorted order. We compare invariants
(KE, COM, total momentum, sorted-x norm) which are relabeling-invariant.
...
def make_initial(n=2000, G=16):
    rng = np.random.RandomState(42)
    pts = 0.25 + 0.5 * rng.rand(n, 3)
